...
| B15 | niedrig | `src/MessstellenController.php` | **Die Tendenz entfällt lautlos.** `letzter_messwert` entsteht über einen Verbund mit **exakter Gleichheit** des Zeitpunkts (`a.letzter_zeitpunkt = w.zeitpunkt`). Trifft er nicht, wird `{TENDENZ}` durch eine leere Zeichenkette ersetzt — der Beitrag geht ohne Tendenzangabe hinaus, mit einem übrig gebliebenen Leerzeichen, und niemand bemerkt es. Produktiv tritt das praktisch nur einmal auf, nämlich bevor der erste Versand `letzter_zeitpunkt` auf einen echten Messzeitpunkt zieht; danach heilt es sich selbst. Erwägenswert wäre ein Ersatztext für `{TENDENZ}` statt des Entfallens. Die Protokollausgabe benennt einen fehlenden Vorwert seit dem 14.08.2026 als `(keiner)`. |
...
| ~~B16~~ | **behoben** 14.08.2026 | `src/MessstellenController.php` | **Zeitangabe ohne Zeitzone.** Die Meldung „Lade Messwerte für … seit …" gab den Zeitpunkt in Ortszeit aus, ohne das kenntlich zu machen — die unmittelbar folgenden Meldungen derselben Ausgabe kennzeichnen ihre Zeitpunkte dagegen als UTC. Wer beides nebeneinander las, musste die Angaben für widersprüchlich halten. Die Umrechnung war korrekt, nur die Beschriftung fehlte; Ausgabe und Protokoll hängen jetzt „Ortszeit" an. Die Formatierung liegt in `formatLocalTime()` und ist ohne Datenbank prüfbar. |
